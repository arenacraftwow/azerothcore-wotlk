...
const char* CONF_mpq_list[] = {
    "common.MPQ", "common-2.MPQ", "lichking.MPQ", "expansion.MPQ", "patch.MPQ", "patch-2.MPQ", "patch-3.MPQ", "patch-5.MPQ",
};
    ...
    for (uint32 z = 0; z < map_count; ++z)
    {
        ...
        for (uint32 y = 0; y < WDT_MAP_SIZE; ++y)
        {
            for (uint32 x = 0; x < WDT_MAP_SIZE; ++x)
            {
                ...
                if (!ConvertADT(mpqFileName, outputFileName, y, x, build))
                {
                    printf("\nFailed to ConvertADT %s\n", mpqFileName.c_str());
                }
            }
            // draw progress bar
...
void LoadCommonMPQFiles()
{
    ...
    for (int i = 0; i < count; ++i)
    {
        ...
        {
            printf("#LoadCommonMPQFiles: does not exist %s", filename);
        }
    }
}
